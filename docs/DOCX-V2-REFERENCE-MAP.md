...
The current seam remains a valid compatibility boundary because V1 remains the
default. V2 now has its own page-flow orchestration, measurement refinement,
invalidation, geometry, post-render diagnostics, and probe support while
reusing GenOffice pagination primitives. It does not touch ProseMirror state,
`blocksToPmDoc`, `pmDocToSavePlan`, dirty tracking, save, reparse, or the
protected DOCX engine.

The important limitation is equally clear: V2 is still experimental and V1 is
still the fallback baseline. The implementation exposes substantially more than
page-slice selection, but default-readiness still requires repeatable proof for
line/table geometry, page gaps, floats, header/footer overlays, position mapping,
and save/reopen preservation across the representative corpus.
...
No component is recommended for `PORT_FROM_CASUAL` in this audit. The next
highest-value work is to run and extend the existing parity/diagnostics harness
until V2 default-readiness is demonstrated; no independent paginator or painter
replacement is justified by this reference comparison.
...
| 20. Visual regression / fidelity testing           | Existing Docs pagination/parity/gap tests and DOCX fixture tests cover geometry and preservation; V2 unit and e2e parity tests compare the experimental path with V1. | e2e `visual-regression.spec.ts` snapshots plus section pagination, kinsoku, table, float, and performance fixtures.                                                                                | layout-bridge visual/line-cache tests, engine tests, and performance baselines.                                                                                  | Casual has the most directly reusable testing pattern: fixture-driven screenshots plus focused feature fixtures. Extend GenOffice’s existing contracts; do not copy the test harness wholesale.                                   | `LEARN_FROM_CASUAL`   |
...
## 13. Recommended next task after this audit

Run and extend the existing renderer parity diagnostics harness at the
`presentation-v2` seam. V1 remains the default and V2 remains experimental.

The harness should continue to run the same representative Docs fixtures
through both selectors and compare:
...
This is the smallest next task because source already contains the V2
orchestration, geometry, post-render diagnostics, and browser parity surfaces.
The remaining work is proof and coverage, not a new layout algorithm. Keep V1
available and do not independently rewrite a paginator or painter until the
parity gates in `DOCX-PRESENTATION-V2.md` pass.
