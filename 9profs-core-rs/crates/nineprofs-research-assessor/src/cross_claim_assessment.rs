...
impl CrossClaimConsistencyAssessorConfig {
    ...
    pub fn from_env() -> Self {
        let provider = std::env::var("NINEPROFS_CROSS_CLAIM_CONSISTENCY_ASSESSOR_PROVIDER")
            .unwrap_or_default();
        let model =
            std::env::var("NINEPROFS_CROSS_CLAIM_CONSISTENCY_ASSESSOR_MODEL").unwrap_or_default();
        let base_url = std::env::var("NINEPROFS_CROSS_CLAIM_CONSISTENCY_ASSESSOR_BASE_URL").ok();
        let api_key_env = std::env::var("NINEPROFS_CROSS_CLAIM_CONSISTENCY_ASSESSOR_API_KEY_ENV")
            .unwrap_or_default();
        let mut config = Self::new(provider, model, base_url, api_key_env);
        if let Ok(value) = std::env::var("NINEPROFS_CROSS_CLAIM_CONSISTENCY_ASSESSOR_TIMEOUT_MS")
